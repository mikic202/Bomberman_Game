...
TEST_CASE("Tests for enemy")
{
	SECTION("Get available direction tests")
	{
		...
		auto directions = enemy.get_available_directions(walls, std::vector<std::shared_ptr<Bomb> >(), std::vector<std::shared_ptr<Player> >());
		...
		directions = enemy.get_available_directions(walls, std::vector<std::shared_ptr<Bomb> >(), std::vector<std::shared_ptr<Player> >());
