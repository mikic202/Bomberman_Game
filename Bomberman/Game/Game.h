...
private:
	...
	void play_coop_();
	void play_versus_();
	void move_player_(std::shared_ptr< Player> player, std::vector<std::shared_ptr<Wall> > items_on_b, sf::RenderWindow& window, std::vector<sf::Keyboard::Key> keys);
	void place_bombs_(std::shared_ptr< Player> player);
